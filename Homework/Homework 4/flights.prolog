...
%One to one from flight ~ Direct FLight
route(Departing, Arriving, Visited, DepartureTime, ArrivalTime, Flights, TotalCost) :- 
  flight(Departing, Arriving, DepartureTime, ArrivalTime, TotalCost), Flights = [Departing, Arriving],
  \+ member(Arriving, Visited).


%Stop to transfer other flights ~ Layover Flight
route(Departing, Arriving, Visited, DepartureTime, ArrivalTime, Flights, TotalCost) :-
    flight(Departing, NextFlight, DepartureTime, NextArrival, InitialCost),
    \+ member(NextFlight, Visited), append(Visited, [Departing, NextFlight], NewVisited),
    route(NextFlight, Arriving, NewVisited, NextTime, ArrivalTime, FlightList, FlightCost),
    NextArrival @< NextTime, Flights = [Departing|FlightList],
    TotalCost is InitialCost + FlightCost.



%Test Cases-----------------------------------------
%DirectFlights:
%route(sjc, lax, [], DT, AT, Flights, TotalCost). 
%route(phl, sfo, [], DT, AT, Flights, TotalCost). 
...
%route(phl, lax, [], DT, AT, Flights, TotalCost).
